                ...
                <div style="height:10px;"></div>
						...
						<tbody style="display: block;overflow:auto; width: 100%;height: 435px; word-break: break-word; scrollbar-width: none; " >
							<?php
                                  ...
                                  $ter_ =  mysqli_fetch_array($query_ter);
                                  $ter_ad = ($ter_['descrip']);
                                  $query = mysqli_query($conn," SELECT a.que_id as que_id, a.trip_no as trip_no, a.bus_no as bus_no,CONCAT(d.l_name,' ',d.f_name) as dri, CONCAT(e.l_name,' ',e.f_name) as con, b.descrip as type_descrip, b.abbr as abbr, a.seat_cap, a.from_ter as from_ter, a.to_ter as to_ter, time_format(a.que_time, '%h:%i %p') as que_time, a.que_date as que_date, f.descrip as stat, f.que_stat_arr_id as que_stat_, a.que_lat as que_lat, a.que_long as que_long  FROM que_details a, bus_type b, ter_details c, user_dri d, user_con e, que_stat_arr f WHERE a.que_stat_arr_id = f.que_stat_arr_id AND a.dri_id = d.dri_id AND a.con_id = e.con_id AND  a.bus_type_id = b.bus_type_id AND to_ter = '$ter_ad'  GROUP BY que_id ORDER BY que_stat_ desc   ");
...
<script>
var map_op = document.getElementById("myBtn");
</script>
